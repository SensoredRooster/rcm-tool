...
const TESTER_UPLOAD_FOLDERS = new Set(["Tester Uploads", "VODs", "Screenshots", "Bug Reports", "Logs"]);
const META_LATEST = "__portal/latest.json";
const ADMIN_PASSWORD_HASH = "eb53f5a437ed0de68d8b2c3c680852fee36adeea2e22a9a5c0ba8d0d910b3b55";
const TESTER_PASSWORD_HASH = "01185f7f9531ff095b4af72ef514f2f97df0d5dc6ae3c4173853efd6fa455993";
...
const LOGIN = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RCMTool Tester Share</title>
...
const PORTAL = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RCMTool Tester Share</title>
...
<body><header><div class="brand"><strong>RCMTool Tester Share</strong><span id="role"></span></div><button id="logout">Sign out</button></header><div class="wrap"><div id="latest" class="latest hidden"></div><div class="grid"><aside class="panel folders" id="folders"></aside><section class="panel"><div class="toolbar"><input id="file" type="file"><input id="desc" placeholder="Optional description"><button class="primary" id="upload">Upload</button><span class="muted" id="status"></span></div><div class="progress hidden" id="progress"><i></i></div><table class="files"><thead><tr><th>Name</th><th>Size</th><th>Uploaded</th><th>Description</th><th>Actions</th></tr></thead><tbody id="rows"></tbody></table></section></div></div>
<script>let role="tester",folder="Releases";const folders=["Releases","Tester Uploads","VODs","Screenshots","Bug Reports","Logs","Archived"];const fmt=n=>n<1024?n+" B":n<1048576?(n/1024).toFixed(1)+" KB":n<1073741824?(n/1048576).toFixed(1)+" MB":(n/1073741824).toFixed(1)+" GB";async function api(url,opt){const r=await fetch(url,opt);if(r.status===401){location.href="/login";throw new Error("Unauthorized")}const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||("HTTP "+r.status));return d}
...
export default { async fetch(request, env) {
  ...
  if (request.method === "GET" && url.pathname === "/health") return json({ ok:true, service:"rcm-tool-tester-share", storage:"r2" });
