...
async function signSession(env, role) {
  const payload = b64url(encoder.encode(JSON.stringify({ role, exp: Math.floor(Date.now()/1000)+SESSION_SECONDS, project: "rcm-tool" })));
...
async function verifySession(request, env) {
  ...
  try {
    ...
    if (payload.project !== "rcm-tool" || payload.exp < Math.floor(Date.now()/1000)) return null;
