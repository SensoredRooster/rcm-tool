...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, service: "rcm-tool-support-collector" });
    }
    if (request.method === "POST" && url.pathname === "/upload") {
      ...
      const session = safeName(request.headers.get("x-rcm-session"), "unknown-session");
      const original = safeName(request.headers.get("x-rcm-filename"), "RCMTool-Support.zip");
